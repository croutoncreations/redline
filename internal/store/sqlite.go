...
func (d *DB) SaveSnapshot(ctx context.Context, s decision.UsageSnapshot, raw []byte) error {
	...
	if s.Short != nil {
		...
		shortReset = formatTokenTime(s.Short.ResetsAt)
	}
	tx, err := d.db.BeginTx(ctx, nil)
	if err != nil {
...
weekly_remaining, weekly_resets_at, source, confidence, raw_payload
	...
	result, err := tx.ExecContext(
		...
		query,
		s.Provider,
		formatTokenTime(s.ObservedAt),
		shortRemaining,
		shortReset,
		s.Weekly.Remaining,
		formatTokenTime(s.Weekly.ResetsAt),
		s.Source,
		s.Confidence,
		raw,
	)
...
snapshot_id, pool_key, source_label, scope, role, remaining, resets_at, period_duration_s, reset_inferred
			...
			formatTokenTime(allowance.ResetsAt), allowance.PeriodDurationSeconds, allowance.ResetInferred)
		if err != nil {
